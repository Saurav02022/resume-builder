...
> **Resume AI Agent:** Sections **§1–§9** match [`notion/DESIGN.md`](./notion/DESIGN.md) (keep in sync). **§10** is repo-only.
...
## 10. Resume AI Agent — Implementation (this repo only)

*This section is **not** part of the upstream getdesign Notion file. It maps §1–§9 above to this codebase. If anything conflicts, **§1–§9 above and [`notion/DESIGN.md`](./notion/DESIGN.md) take precedence** — update code, not the spec.*

### Code map

| Spec area | Implementation |
|-----------|------------------|
| **§2 Colors & §6 Soft Card shadow** | `app/globals.css` — `:root` / `.dark` CSS variables; `--shadow-notion-card` equals **Card Shadow** (light); dark mode uses an adapted 4-layer stack. |
| **§3 Font** | `app/layout.tsx` — **Inter** (NotionInter substitute), weights 400–700. `--font-heading` = `--font-sans`. |
| **§3 OpenType** | `globals.css` `body` — `kern`, `liga`, `lnum`, `locl`. |
| **§4 Primary / Secondary / Ghost / Link buttons** | `components/ui/button.tsx` — `rounded-[4px]` (Micro 4px); primary `bg-primary` / `hover:bg-primary-hover`; **hover `scale(1.05)`**, **active `scale(0.9)`** per §4 & §8; `focus-visible:ring-2`. |
| **§4 Cards** | `components/ui/card.tsx` — Comfortable **12px** (`--radius`), whisper `border-border`, `--shadow-notion-card`. |
| **§4 Inputs** | `components/ui/textarea.tsx` — 4px radius, `#dddddd` border via `--input`, placeholder `#a39e98`. |
| **§4 Pill Badge** | `components/ui/badge.tsx` — **`rounded-full`** (9999px), **12px** / **font-semibold** / **0.125px** tracking on secondary pill; colors `#f2f9ff` / `#097fe8`. |
| **§5 Radius scale** | Buttons/inputs: **4px**; standard cards: **12px** via `--radius`; pills: **full** on badges. |
| **Semantic Green §2** | Stepper done, diff “add”, fix blocks: **`#1aae39`**. |
| **Wizard shell** | `components/wizard/wizard-shell.tsx` — max width `1920px` (tool layout; Notion marketing uses ~1200px — §5 Grid). Warm `#f6f5f4` radial wash. Sticky header (Notion §4 Nav describes non-sticky marketing nav — product choice). |
| **Stepper** | `components/wizard/wizard-stepper.tsx`. |
| **Monospace (code / `.tex`)** | Geist Mono — not in Notion spec; used only for technical strings. |
| **Errors** | `--destructive` (`#dc2626` / `#ef4444`) — app chrome; not in Notion core palette. |
| **§2 Deep Navy** | Reserved for future emphasis; not wired in v1 components. |

### Maintenance

- Refresh upstream: `npx getdesign@latest add notion` → replaces `notion/DESIGN.md`.
- **Sync rule:** `design.md` §1–§9 must stay **identical** to `notion/DESIGN.md`. After refreshing, copy `notion/DESIGN.md` into `design.md` and re-append **§10** (this section), or run the repo sync script if one exists.

### Routes

`/resume/job` → `/resume/review` → `/resume/export`; layout `app/resume/layout.tsx` → `WizardShell`.
